...
- **BL-S3-06:** local `wrangler dev` fails because `apps/server/src/index.ts` uses `@neondatabase/serverless` (neon-http driver) under workerd but local `DATABASE_URL` is a plain Postgres TCP endpoint (`localhost:5432`). Better-auth user-lookup queries 500 with `NeonDbError: Network connection lost`; every `protectedProcedure` then 401s; F1 displays "No results" without an error UI. Discovered live via `agent-browser` driving the UI 2026-05-08. Three options for the fix: (a) switch to drizzle's `pg-proxy` driver behind a feature flag for local dev; (b) add a Hyperdrive binding pointing at local Postgres; (c) gate the dev path through Alchemy run (which uses `compatibility: "node"` and Node's native pg). Earliest landing: any sprint with infra-polish scope. **Surfaces as a real demo blocker** — sprint reviews depending on `wrangler dev` UI walk-through must use Alchemy run instead until BL-S3-06 lands.
